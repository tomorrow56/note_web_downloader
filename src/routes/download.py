...
def remove_data_uri_images(markdown_content):
    """Remove image references with data URIs from Markdown."""
    # Remove markdown image syntax with data: URIs
    # Matches: ![alt text](data:image/...)
    markdown_content = re.sub(r'!\[.*?\]\(data:[^)]+\)', '', markdown_content)
...
def download_note_article(url, output_dir="."):
    ...
    for selector in selectors:
        if isinstance(selector, tuple):
            if selector[0] == 'meta':
                tag = soup.find('meta', attrs=selector[1])
                if tag and tag.get('content'):
                    title = tag.get('content').strip()
                    logger.info(f"Found article title from meta tag: {title}")
                    break
            elif selector[0] == 'title':
                tag = soup.find('title')
                if tag:
                    title = tag.get_text().strip()
                    logger.info(f"Found article title from title tag: {title}")
                    break
        else:
            tag = soup.find(selector)
            if tag:
                title = tag.get_text().strip()
                logger.info(f"Found article title from {selector} tag: {title}")
                break
    ...
    logger.info("Removing data URI images from Markdown...")
    markdown_content = remove_data_uri_images(markdown_content)
